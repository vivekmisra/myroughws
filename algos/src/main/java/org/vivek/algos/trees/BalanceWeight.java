...
public class BalanceWeight {
	...
	public static double doBalance(Balance balance) {
		System.out.println("doBalance called for Balance ="+ balance);
		if (balance == null)
			return 0;
		if (balance.left == null && balance.right == null) {
			...
			return printBalance(balance.id, balance.leftWeight+ doBalance(balance.left), balance.rightWeight+ doBalance(balance.right));
		}
	}

	public static double printBalance(int id, double leftWeight,
			double rightWeight) {
		System.out.println("doBalance called  printBalance for ="+ id);
...
class Balance {
	...
	public String toString() {
		return "Balance [id=" + id + ", left=" + left + ", right=" + right
